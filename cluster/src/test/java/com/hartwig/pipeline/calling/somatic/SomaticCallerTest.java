...
public class SomaticCallerTest extends TertiaryStageTest<SomaticCallerOutput> {
    ...
    protected Stage<SomaticCallerOutput, SomaticRunMetadata> createVictim() {
        return new SomaticCaller(TestInputs.defaultPair(), false);
    }

    @Override
    protected List<String> expectedCommands() {
        return ImmutableList.of(
                "java -Xmx8G -cp /opt/tools/sage/1.1/sage.jar com.hartwig.hmftools.sage.SageHotspotApplication -tumor tumor -tumor_bam /data/input/tumor.bam -reference reference -reference_bam /data/input/reference.bam -known_hotspots /opt/resources/sage/KnownHotspots.tsv -coding_regions /opt/resources/sage/CodingRegions.bed -ref_genome /opt/resources/reference_genome/Homo_sapiens.GRCh37.GATK.illumina.fasta -out /data/output/tumor.sage.hotspots.vcf.gz",
                ...
                "/opt/tools/bcftools/1.3.1/bcftools annotate -a /opt/resources/sage/SAGE_PON.vcf.gz -c SAGE_PON_COUNT -o /data/output/tumor.sage.hotspots.pon.annotated.vcf.gz -O z /data/output/tumor.sage.hotspots.filtered.vcf.gz",
                ...
                "unzip -d /opt/resources /opt/resources/snpeff/snpEff_v4_3_GRCh37.75.zip",
                "/opt/tools/strelka/1.0.14/bin/configureStrelkaWorkflow.pl --tumor /data/input/tumor.bam --normal /data/input/reference.bam --config /opt/resources/strelka_config/strelka_config_bwa_genome.ini --ref /opt/resources/reference_genome/Homo_sapiens.GRCh37.GATK.illumina.fasta --output-dir /data/output/strelkaAnalysis",
                "make -C /data/output/strelkaAnalysis -j $(grep -c '^processor' /proc/cpuinfo)",
                "java -Xmx20G -jar /opt/tools/gatk/3.8.0/GenomeAnalysisTK.jar -T CombineVariants -R /opt/resources/reference_genome/Homo_sapiens.GRCh37.GATK.illumina.fasta --genotypemergeoption unsorted -V:snvs /data/output/strelkaAnalysis/results/passed.somatic.snvs.vcf -V:indels /data/output/strelkaAnalysis/results/passed.somatic.indels.vcf -o /data/output/tumor.strelka.vcf",
                "/opt/tools/bcftools/1.3.1/bcftools annotate -a /opt/resources/mappability/out_150_hg19.mappability.bed.gz -h /opt/resources/mappability/mappability.hdr -c CHROM,FROM,TO,-,MAPPABILITY -o /data/output/tumor.mappability.annotated.vcf.gz -O z /data/output/tumor.strelka.vcf",
                "/opt/tools/tabix/0.2.6/tabix /data/output/tumor.mappability.annotated.vcf.gz -p vcf",
                "/opt/tools/bcftools/1.3.1/bcftools annotate -a /opt/resources/pon/GERMLINE_PON.vcf.gz -c GERMLINE_PON_COUNT -o /data/output/tumor.germline.pon.annotated.vcf.gz -O z /data/output/tumor.mappability.annotated.vcf.gz",
                "/opt/tools/tabix/0.2.6/tabix /data/output/tumor.germline.pon.annotated.vcf.gz -p vcf",
                "/opt/tools/bcftools/1.3.1/bcftools annotate -a /opt/resources/pon/SOMATIC_PON.vcf.gz -c SOMATIC_PON_COUNT -o /data/output/tumor.somatic.pon.annotated.vcf.gz -O z /data/output/tumor.germline.pon.annotated.vcf.gz",
                "/opt/tools/tabix/0.2.6/tabix /data/output/tumor.somatic.pon.annotated.vcf.gz -p vcf",
                "java -Xmx20G -jar /opt/tools/strelka-post-process/1.6/strelka-post-process.jar -v /data/output/tumor.somatic.pon.annotated.vcf.gz -hc_bed /opt/resources/beds/NA12878_GIAB_highconf_IllFB-IllGATKHC-CG-Ion-Solid_ALLCHROM_v3.2.2_highconf.bed -t tumor -o /data/output/tumor.strelka.post.processed.vcf.gz -b /data/input/tumor.bam",
                ...
                "java -Xmx8G -cp /opt/tools/sage/1.1/sage.jar com.hartwig.hmftools.sage.SageHotspotAnnotation -source_vcf /data/output/tumor.pon.filtered.vcf.gz -hotspot_vcf /data/output/tumor.sage.pon.filter.vcf.gz -known_hotspots /opt/resources/sage/KnownHotspots.tsv -out /data/output/tumor.sage.hotspots.annotated.vcf.gz",
